...
RUN \
  ...
  CHROME_RELEASE=$(curl -sX GET "https://googlechromelabs.github.io/chrome-for-testing/known-good-versions.json" | jq -r '.versions | map(select(.version | startswith("119."))) | max_by(.version) | .version') && \

...
class SetEnvs():
    ...
    def check_env(self) -> None:
        ...
        try:
            ...
            self.container: str = os.environ['CONTAINER']
...
class CI(SetEnvs):
    ...
    def __init__(self) -> None:
        ...
        self.outdir: str = f"{os.path.dirname(os.path.realpath(__file__))}/output/{self.container}/{self.meta_tag}"
    ...
    def report_render(self) -> None:
        ...
        env = Environment(autoescape=select_autoescape(enabled_extensions=("html", "xml"), default_for_string=True), loader=FileSystemLoader(os.path.dirname(os.path.realpath(__file__))))
    ...
    def report_upload(self) -> None:
        ...
        self.logger.info("Report available on https://%s/%s/%s/index.html", self.bucket, self.container, self.meta_tag)
    ...
    def upload_file(self, file_path: str, object_name: str, content_type: dict) -> None:
        ...
        latest_dir: str = f"{self.container}/latest-{self.branch}"
    ...
    def log_upload(self) -> None:
        ...
        try:
            self.upload_file(f"{self.outdir}/ci.log", "ci.log", {"ContentType": "text/plain"})
            with open(f"{self.outdir}/ci.log", "r", encoding="utf-8") as logs:
                blob: str = logs.read()
                self.create_html_ansi_file(blob, "python", "log")
                self.upload_file(f"{self.outdir}/python.log.html", "python.log.html", {"ContentType": "text/html"})
    ...
    def create_s3_client(self) -> boto3.client:
        ...
        s3_client = boto3.Session().client(
            ...
            endpoint_url="https://ff6f87cc1940578fbe957a7b39b0ae72.r2.cloudflarestorage.com")
        return s3_client

...
def testing(func: Callable):
    ...
    def wrapper(*args, **kwargs) -> Any | None:
        if os.environ.get("DRY_RUN") == "true":
            logger.warning(
                "Dry run enabled, skipping execution of %s", func.__name__)
            return
...
class CI(SetEnvs):
    ...
    def __init__(self) -> None:
        ...
        self.tag_report_tests: dict[str, dict[str, dict]] = {
            tag: {"test": {}} for tag in self.tags}
    ...
    def container_test(self, tag: str) -> None:
        ...
        build_info: dict = self.get_build_info(
            container, tag)  # Get the image build info
    ...
    def get_build_info(self, container: Container, tag: str) -> dict[str, str]:
        ...
        try:
            ...
            build_info = {"version": "ERROR", "created": "ERROR",
                          "size": "ERROR", "maintainer": "ERROR"}
    ...
    def report_upload(self) -> None:
        ...
        for filename in os.listdir(self.outdir):
            time.sleep(0.5)
            ctype: tuple[str | None, str | None] = mimetypes.guess_type(
                filename.lower(), strict=False)
        ...
        self.logger.info("Report available on https://%s/%s/%s/index.html",
                         self.bucket, self.container, self.meta_tag)
    ...
    def take_screenshot(self, container: Container, tag: str) -> None:
        ...
        '''
        proto: Literal["https", "http"] = "https" if self.ssl.upper(
        ) == "TRUE" else "http"
